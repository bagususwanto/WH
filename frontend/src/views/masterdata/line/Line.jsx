...
const Line = () => {
  ...
  const apiLine = 'line'
  const apiLineDelete = 'line-delete'
  ...
  const getLines = async () => {
    try {
      ...
      setLines(dataWithFormattedFields)
    } catch (error) {
  ...
  const actionBodyTemplate = (rowData) => (
    <div style={{ display: 'flex', gap: '10px' }}>
      <Button
        ...
        onClick={() => handleEditLine(rowData)}
      />
      <Button
        label="Delete"
        icon="pi pi-trash"
  ...
  const handleDeleteLine = (lineId) => {
    MySwal.fire({
      title: 'Are you sure?',
      text: 'This line cannot be recovered!',
      icon: 'question',
  ...
  const confirmDelete = async (lineId) => {
    try {
      await deleteMasterDataById(apiLineDelete, lineId)
      MySwal.fire('Deleted!', 'Line deleted successfully.', 'success')
      setShouldFetch(true)
    } catch (error) {
  ...
  const validateLine = (line) => {
    const requiredFields = [{ field: 'lineName', message: 'Line is required' }]
  ...
  const handleSaveLine = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await updateMasterDataById(apiLine, currentLine.id, lineToSave)
        MySwal.fire('Updated!', 'Line has been updated.', 'success')
      } else {
        delete lineToSave.id
        await postMasterData(apiLine, lineToSave)
        MySwal.fire('Added!', 'Line has been added.', 'success')
      }
    } catch (error) {
  ...
  const exportExcel = () => {
    import('xlsx').then((xlsx) => {
      const mappedData = lines.map((item, index) => ({
        No: index + 1,
        Line: item.lineName,
      ...
      saveAsExcelFile(excelBuffer, 'master_data_lines')
    })
  }
    ...
    <CRow>
      <CCol>
        <CCard className="mb-4">
          <CCardHeader>Master Data Lines</CCardHeader>
          <CCardBody>
            {loading ? (
              <LoadingComponent />
            ) : (
      ...
      <CModal backdrop="static"  visible={modal} onClose={() => setModal(false)}>
        <CModalHeader onClose={() => setModal(false)}>
          <CModalTitle>{isEdit ? 'Edit Line' : 'Add Line'}</CModalTitle>
        </CModalHeader>
        <CModalBody>
          <CForm>
            <CRow>
        ...
        <CModalFooter>
          ...
          >
            <CButton color="primary" onClick={handleSaveLine}>
              {loading ? (
                <>
